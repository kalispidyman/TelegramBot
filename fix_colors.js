...
import { Key, Lock, Edit2, Eye, EyeOff, Brain, Package, Settings, Zap } from 'lucide-react';
...
const CredentialManagerUI = ({ ctx, isMobile = false }) => {
    ...
    const providers = [
        { id: 'boa', name: 'Bay of Assets', icon: <Package className="w-5 h-5" />, color: 'emerald', key: 'hasApiKey' },
    ...
    const getProviderConfig = (id) => {
        switch(id) {
            case 'boa': return {
                isEditing: isEditingKey, setIsEditing: setIsEditingKey, isSaving: isSavingKey, handleSave: handleSaveKey, 
                keyVal: boaKey, setKeyVal: setBoaKey, showKeyInput: showBoaKeyInput, setShowKeyInput: setShowBoaKeyInput, 
                revealConfigured: revealBoaConfigured, setRevealConfigured: setRevealBoaConfigured,
                hasKey: session.hasApiKey, rawKey: session.rawApiKey, maskedKey: session.maskedApiKey,
                fields: [{ name: 'API Key', val: boaKey, setVal: setBoaKey }]
            };
        ...
        <div className={\`flex flex-col gap-4 animate-in fade-in duration-300 w-full \${isMobile ? 'px-2' : ''}\`}>
            ...
            {!isMobile && (
                <div className="flex items-center justify-between">
                    <h4 className="text-[11px] font-bold tracking-[0.3em] text-transparent bg-clip-text bg-gradient-to-r from-white to-white/40 uppercase drop-shadow-[0_0_10px_rgba(255,255,255,0.1)]">
                        Quantum Config
                    </h4>
                </div>
            )}
            ...
            <div className={\`flex flex-col bg-black/40 border \${s.panelBorder} rounded-3xl overflow-hidden backdrop-blur-2xl relative animate-in slide-in-from-top-4 duration-300\`}>
                ...
                <div className={\`flex items-center justify-between px-5 py-4 bg-gradient-to-r \${s.headerBg} to-transparent border-b\`}>
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        {!cfg.hasKey && !cfg.isEditing && (
                            <button 
                                onClick={() => { cfg.setIsEditing(true); cfg.setShowKeyInput(true); }}
                ...
                <div className="p-5 flex flex-col gap-5 relative">
                    ...
                    {activePasswordCard === activeProvider.id && (
                        <div className="absolute inset-0 z-20 flex flex-col items-center justify-center p-6 bg-black/80 backdrop-blur-md animate-in fade-in duration-200">
...
`;
fs.writeFileSync('src/components/CredentialManagerUI.jsx', code);
